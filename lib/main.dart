import 'package:dev_quiz/ui/routes/knowledge_base_route.dart';
import 'package:dev_quiz/ui/routes/main_route.dart';
import 'package:dev_quiz/ui/routes/playground_route.dart';
import 'package:dev_quiz/ui/routes/quiz_route.dart';
import 'package:dev_quiz/ui/routes/webview_route.dart';
import 'package:flutter/material.dart';

void main() {
  runApp(const MyApp());
}

class MyApp extends StatelessWidget {
  const MyApp({Key? key}) : super(key: key);

  // This widget is the root of your application.
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Flutter Demo',
      theme: ThemeData(
        // This is the theme of your application.
        //
        // Try running your application with "flutter run". You'll see the
        // application has a blue toolbar. Then, without quitting the app, try
        // changing the primarySwatch below to Colors.green and then invoke
        // "hot reload" (press "r" in the console where you ran "flutter run",
        // or simply save your changes to "hot reload" in a Flutter IDE).
        // Notice that the counter didn't reset back to zero; the application
        // is not restarted.
        primarySwatch: Colors.blue,
      ),
      // home: const MyHomePage(title: 'Flutter Demo Home Page'),
      initialRoute: MainRoute.routeName,
      routes: {
        MainRoute.routeName: (context) => MainRoute(),
        WebviewRoute.routeName: (context) => WebviewRoute(),
        PlaygroundRoute.routeName: (context) => PlaygroundRoute(),
        QuizRoute.routeName: (context) => QuizRoute(),
        KnowledgeBaseRoute.routeName: (context) => KnowledgeBaseRoute()
      },
    );
  }
}

